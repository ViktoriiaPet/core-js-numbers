 ...
 */
function getDistanceBetweenPoints(x, y, z, b) {
  let a = 0;
  a = ((z - x) ** 2 + (b - y) ** 2) ** (1 / 2);
  return a;
}
 ...
 * '-525.5'     => -525.5
 */
function parseNumberFromString(x) {
  const a = Number(x);
  return a;
}
...
function roundToPowerOfTen(num, pow) {
  const a = Math.round(num / 10 ** pow) * 10 ** pow;
  return a;
}
...
function toExponential(number, fractionDigits) {
  const result = Number.parseFloat(number).toExponential(fractionDigits);
  return result;
}
...
function toFixed(number, fractionDigits) {
  const result = Number.parseFloat(number).toFixed(fractionDigits);
  return result;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = x1 + x2 + x3;
  const result = toFixed(sum, String(x1).length + 1);
  return result;
}
